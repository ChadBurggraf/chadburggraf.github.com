...
* "Learn more »":virtual-keychain

h2. Tasty Libraries

My *monolithic* general purpose C# library. Against standard wisdom, I'm packing every reusable module I write into one massive DLL so I can easily drop it into new projects. Includes things like convenience extension methods, a Google geocoding client library, a background job queue and processor, a URL token library and a bunch more. My C# kitchen sink.

- Get the "source code (GitHub)":http://github.com/chadburggraf/tasty
- "Documentation »":tasty-libraries

h2. SThreeQL

A rudimentary "Amazon S3":http://aws.amazon.com/s3/ -based Microsoft SQL Server backup/restore utility. 

*Use case:* you're a consultant or contractor managing a number of small-to-medium client sites deployed on a couple of private servers. Automatically back up all of the SQL Server databases on a schedule of your choosing to Amazon S3. Optionally restore them periodically to your staging server, and enable your developers to restore the latest backups easily to their development machines.

- Get the "source code (GitHub)":http://github.com/chadburggraf/sthreeql
- "Documentation »":sthreeql

h2. Kayjax

An OO(Object Oriented) C# Ajax library. This replaced a previous RPC(Remote Procedure Call) -style library I had written when C# 3.0 and WCF(Windows Communication Foundation) came out. This project provides a super lightweight alternative to creating JSON(JavaScript Object Notation) API(Application Programming Interface)s using ASP.NET MVC or similar.

- Get the "source code (GitHub)":http://github.com/chadburggraf/kayjax
- "Documentation »":kayjax
...
- Get the "source code (GitHub)":http://github.com/chadburggraf/wmd
- "Documentation »":wmd